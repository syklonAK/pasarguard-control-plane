...
def section_menu(chat_id):
 if chat_id==base.root_id():
  return {"inline_keyboard":[[{"text":"📊 داشبورد","callback_data":"menu|dashboard"},{"text":"🖥 سرورها","callback_data":"menu|servers"}],[{"text":"👥 نمایندگان","callback_data":"menu|resellers"},{"text":"💰 درخواست‌های مالی","callback_data":"menu|finance"}],[{"text":"⚙️ مدیریت سیستم","callback_data":"menu|system"}],[{"text":"🌐 وب‌اپ پیشرفته","web_app":{"url":base.os.getenv("WEBAPP_URL","")}}]]}
 return {"inline_keyboard":[[{"text":"🏠 حساب من","callback_data":"menu|dashboard"},{"text":"💳 کیف پول","callback_data":"menu|wallet"}],[{"text":"📈 مصرف","callback_data":"menu|usage"},{"text":"👥 زیرمجموعه‌ها","callback_data":"menu|resellers"}],[{"text":"📜 تراکنش‌ها","callback_data":"menu|transactions"},{"text":"🛟 پشتیبانی","callback_data":"menu|support"}],[{"text":"🌐 پنل کاربری","web_app":{"url":base.os.getenv("WEBAPP_URL","")}}]]}
...
async def main():
 original_message=base.handle_message;original_callback=base.handle_callback
 async def handle_message(redis,chat_id,value):
  value=(value or "").strip()
  if value in ("/menu","منو","🔙 منوی اصلی"):await show_section_menu(chat_id);return
  aliases={"/dashboard":"🏠 داشبورد مدیریت" if chat_id==base.root_id() else "🏠 حساب من","/servers":"🖥 مدیریت سرورها" if chat_id==base.root_id() else "🏠 حساب من","/support":"⚙️ مدیریت سیستم" if chat_id==base.root_id() else "🛟 پشتیبانی"}
  await original_message(redis,chat_id,aliases.get(value,value))
 ...
 async def handle_callback(chat_id,callback_id,data):
  ...
  try:
   ...
   if not context:await base.answer_callback(callback_id,"حساب متصل نیست");await base.ensure_linked(chat_id);return
   ...
   await base.answer_callback(callback_id)
  except Exception as exc:print(f"submenu {data}: {exc}",flush=True);await base.answer_callback(callback_id,"عملیات انجام نشد")
 base.handle_message=handle_message;base.handle_callback=handle_callback;await base.main()
