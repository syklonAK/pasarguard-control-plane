...
from sqlalchemy import BigInteger, DateTime, ForeignKey, Integer, String, Text, UniqueConstraint, create_engine, func, select
...
class Organization(Base):
    ...
    credit_limit_irr:Mapped[int]=mapped_column(BigInteger,default=0)
...
class Contract(Base):
    ...
    price_per_gib_irr:Mapped[int]=mapped_column(BigInteger); status:Mapped[str]=mapped_column(String(24),default="active")
class Account(Base):
    __tablename__="accounts"; __table_args__=(UniqueConstraint("owner_key","code"),)
    id:Mapped[str]=mapped_column(String(36),primary_key=True,default=uid)
    owner_key:Mapped[str]=mapped_column(String(80),index=True); code:Mapped[str]=mapped_column(String(40),default="wallet")
...
class Transaction(Base):
    ...
    kind:Mapped[str]=mapped_column(String(40)); reference:Mapped[str]=mapped_column(String(180),default="")
    created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=now)
...
class Panel(Base):
    ...
    api_key_ref:Mapped[str|None]=mapped_column(String(200)); owner_user_ref:Mapped[str|None]=mapped_column(String(200)); owner_pass_ref:Mapped[str|None]=mapped_column(String(200))
    verify_tls:Mapped[bool]=mapped_column(default=True); status:Mapped[str]=mapped_column(String(24),default="active")
class Binding(Base):
    ...
    username:Mapped[str]=mapped_column(String(80)); status:Mapped[str]=mapped_column(String(24),default="active")
...
class Outbox(Base):
    ...
    aggregate_id:Mapped[str]=mapped_column(String(80)); payload:Mapped[str]=mapped_column(Text); status:Mapped[str]=mapped_column(String(24),default="pending")
class Actor(Base):
...
class Membership(Base):
    ...
    role:Mapped[str]=mapped_column(String(40),default="reseller_admin")
    status:Mapped[str]=mapped_column(String(24),default="active")
...
class PanelIn(BaseModel): name:str; base_url:str; api_key_ref:str|None=None; owner_user_ref:str|None=None; owner_pass_ref:str|None=None
...
def account(s:Session,owner:str)->Account:
    ...
    if not a: a=Account(owner_key=owner); s.add(a); s.flush()
    return a
def balance(s:Session,org:str)->int:
    return int(account(s,org).balance_irr)
def transfer(s:Session,source:str,target:str,amount:int,key:str,kind:str,ref:str,enforce=True):
    ...
    locked=s.scalars(select(Account).where(Account.id.in_([sa.id,ta.id])).order_by(Account.id).with_for_update()).all()
    by_id={a.id:a for a in locked};sa,ta=by_id[sa.id],by_id[ta.id]
    old=s.scalar(select(Transaction).where(Transaction.idempotency_key==key))
    if old:return old
    if enforce and source!="SYSTEM":
        org=s.scalar(select(Organization).where(Organization.id==source).with_for_update())
        if not org or sa.balance_irr+org.credit_limit_irr<amount:raise ValueError("insufficient credit")
    sa.balance_irr-=amount;ta.balance_irr+=amount
    tx=Transaction(idempotency_key=key,kind=kind,reference=ref)
...
class BillingHold(ValueError):
    def __init__(self,binding_id,organization_id):
        super().__init__("insufficient credit; billing hold required")
        self.binding_id=binding_id;self.organization_id=organization_id

def record_hold(s:Session,binding_id:str,organization_id:str):
    ...
    if not exists:s.add(Outbox(topic="billing.hold_required",aggregate_id=binding_id,payload=json.dumps({"organization_id":organization_id})))
...
def observe(s:Session,data:UsageIn):
    ...
    if data.lifetime_bytes<cp.lifetime_bytes:
        cp.anomaly_count+=1;s.add(Usage(binding_id=b.id,lifetime_bytes=data.lifetime_bytes,delta_bytes=0,status="regression",observed_at=data.observed_at));return []
    delta=data.lifetime_bytes-cp.lifetime_bytes
    rows=cascade(delta,edges(s,b.organization_id))
    try:
        for x in rows: transfer(s,x.child_id,x.parent_id,x.amount_irr,f"usage:{b.id}:{cp.lifetime_bytes}:{data.lifetime_bytes}:{x.child_id}","usage",b.id)
    except ValueError as exc:
        if str(exc)=="insufficient credit":raise BillingHold(b.id,b.organization_id) from exc
        raise
    cp.lifetime_bytes=data.lifetime_bytes;cp.observed_at=data.observed_at
...
app=FastAPI(title="PasarGuard B2B Control Plane",version="0.3.0")
...
def create_org(x:OrgIn,s:Session=Depends(db)):
    ...
    account(s,o.id);s.commit();return {"id":o.id,"name":o.name,"parent_id":o.parent_id}
...
def fund(org_id:str,x:FundIn,s:Session=Depends(db)):
    ...
    tx=transfer(s,"SYSTEM",org_id,x.amount_irr,x.idempotency_key,"fund",org_id,False);s.commit();return {"transaction_id":tx.id,"balance_irr":balance(s,org_id)}
...
def panel(x:PanelIn,s:Session=Depends(db)):
    p=Panel(**x.model_dump(),verify_tls=True);s.add(p);s.commit();return {"id":p.id,"name":p.name}
...
def binding(x:BindingIn,s:Session=Depends(db)):
    ...
    b=Binding(**x.model_dump());s.add(b);s.commit();return {"id":b.id}
...
def usage(x:UsageIn,s:Session=Depends(db)):
    try:r=observe(s,x);s.commit();return {"settlements":r}
        ...
        s.rollback();record_hold(s,e.binding_id,e.organization_id);s.commit();raise HTTPException(402,str(e))
...
def usage_batch(x:UsageBatchIn,s:Session=Depends(db)):
    ...
    try:
        for item in x.observations:
            results.append({"binding_id":item.binding_id,"settlements":observe(s,item)})
        s.commit()
...
def web_create_child(x:WebChildIn,identity=Depends(web_identity),s:Session=Depends(db)):
    _,m,parent=membership_for(s,identity.user_id)
    ...
    s.add(Contract(parent_id=parent.id,child_id=child.id,price_per_gib_irr=x.price_per_gib_irr));account(s,child.id);s.commit();return {"id":child.id,"name":child.name}
...
def web_funding_request(x:FundingRequestIn,identity=Depends(web_identity),s:Session=Depends(db)):
    actor,_,o=membership_for(s,identity.user_id);req=FundingRequest(organization_id=o.id,actor_id=actor.id,amount_irr=x.amount_irr);s.add(req);s.commit();return {"id":req.id,"status":req.status}
