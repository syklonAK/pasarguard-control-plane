...
def admin_create_reseller(x:AdminResellerIn,identity=Depends(web_identity),s:Session=Depends(db)):
 ...
 if actor and s.scalar(select(Membership.id).where(Membership.actor_id==actor.id,Membership.status=="active")):raise HTTPException(409,"Telegram account is already linked")
 ...
 if not actor:actor=Actor(telegram_id=x.telegram_id,display_name=x.name);s.add(actor);s.flush()
 else:actor.status="active";actor.display_name=actor.display_name or x.name
...
def admin_funding_requests(identity=Depends(web_identity),s:Session=Depends(db)):
 require_root(identity);membership_for(s,identity.user_id);rows=s.execute(select(FundingRequest,Organization,Actor).join(Organization,Organization.id==FundingRequest.organization_id).join(Actor,Actor.id==FundingRequest.actor_id).where(FundingRequest.status=="pending").order_by(FundingRequest.created_at.asc()).limit(100)).all();return [{"id":request.id,"organization":organization.name,"telegram_id":actor.telegram_id,"amount_irr":request.amount_irr,"status":request.status,"created_at":request.created_at} for request,organization,actor in rows]
