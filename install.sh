...
ask(){ local value; read -r -p "$1: " value </dev/tty; printf '%s' "$value"; }
ask_secret(){ local value; read -r -s -p "$1: " value </dev/tty; echo >&2; printf '%s' "$value"; }
...
if [[ ! -f "$INSTALL_DIR/pyproject.toml" ]]; then
  info "Installing project files in $INSTALL_DIR"
  rm -rf "$INSTALL_DIR"
  mkdir -p "$INSTALL_DIR"
  if [[ -n "$SCRIPT_DIR" && -f "$SCRIPT_DIR/pyproject.toml" ]]; then
    cp -a "$SCRIPT_DIR/." "$INSTALL_DIR/"
  else
    git clone "${GITHUB}/${REPO}.git" "$INSTALL_DIR"
  fi
fi
cd "$INSTALL_DIR"
...
if [[ -f .env ]]; then
  info "Existing configuration found; preserving all secrets"
  set -a; source .env; set +a
else
  DOMAIN="${DOMAIN:-$(ask 'Public domain (example: panel.example.com)')}"
  [[ "$DOMAIN" == *.* ]] || die "Enter a valid domain."
  TELEGRAM_BOT_TOKEN="${TELEGRAM_BOT_TOKEN:-$(ask_secret 'Telegram bot token')}"
  [[ "$TELEGRAM_BOT_TOKEN" == *:* ]] || die "Invalid Telegram bot token."
  POSTGRES_PASSWORD="$(random_hex 32)"
  CONTROL_API_KEY="$(random_hex 32)"
  TELEGRAM_WEBHOOK_SECRET="$(random_hex 32)"
  APP_ENCRYPTION_KEY="$(random_hex 32)"
  INITIAL_SETUP_TOKEN="$(random_hex 32)"
  cat > .env <<EOF
DOMAIN=$DOMAIN
...
ensure_env WEBAPP_URL "${HTTPS}${DOMAIN}/app/"
ensure_env APP_ENCRYPTION_KEY "$(random_hex 32)"
ensure_env INITIAL_SETUP_TOKEN "$(random_hex 32)"
ensure_env POSTGRES_CPU_LIMIT 1.0
ensure_env POSTGRES_MEMORY_LIMIT 2G
ensure_env API_CPU_LIMIT 0.75
ensure_env API_MEMORY_LIMIT 1G
ensure_env API_REPLICAS 1
ensure_env API_WORKERS 1
ensure_env TELEGRAM_CONSUMER_REPLICAS 1
set -a; source .env; set +a

configure_telegram(){
  curl -fsS "${TELEGRAM_API}/bot${TELEGRAM_BOT_TOKEN}/setWebhook" \
...
for _ in $(seq 1 120); do
  API_CID="$(docker compose "${COMPOSE[@]}" ps -q api | head -n1)"
  if [[ -n "$API_CID" ]] && docker exec "$API_CID" python -c 'import urllib.request; urllib.request.urlopen("http://127.0.0.1:8000/health",timeout=2)' >/dev/null 2>&1; then
    INTERNAL_READY=1
    break
  fi
  sleep 2
...
if curl -fsS --max-time 10 "${HTTPS}${DOMAIN}/health" >/dev/null 2>&1; then
  ...
  warn "The internal API is healthy, but public HTTPS is unavailable. Check DNS and ports 80/443, then run update.sh."
fi
