...
[[ ${EUID} -eq 0 ]] || die "Run this updater with sudo."
cd "$INSTALL_DIR"
...
[[ -z "$(git status --porcelain --untracked-files=no)" ]] || die "Tracked files contain local changes. Commit or discard them before updating."
...
if curl -fsS --max-time 10 "${HTTPS}${DOMAIN}/health" >/dev/null 2>&1; then
  ...
  warn "Internal services are healthy, but public HTTPS is unavailable."
fi
